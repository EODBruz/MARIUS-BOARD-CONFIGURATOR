...
function Invoke-SelfInstall {
    # Install/refresh script to %APPDATA%\MARIUS — always downloads fresh to prevent doubling
    try {
        if (-not (Test-Path $script:InstallDir)) {
            New-Item -ItemType Directory -Path $script:InstallDir -Force | Out-Null
        }
        ...
        # If AppData file doesn't exist yet, download a clean copy from GitHub
...
function Check-ForUpdates {
    # Skip update check if launched by the updater itself
    if ($NoUpdate) { return }
    ...
    try {
        ...
        # Only proceed if remote version is newer
        if ([version]$latestVersion -le [version]$script:CurrentVersion) { return }
        ...
        # ── Download and overwrite AppData script ─────────────────────────────
        try {
            if (-not (Test-Path $script:InstallDir)) {
                New-Item -ItemType Directory -Path $script:InstallDir -Force | Out-Null
            }
            ...
            # Download to a temp file first, then atomic-replace — prevents corruption
            $tempPath = "$script:InstallPath.tmp"
            $wc2.DownloadFile($script:ScriptUrl, $tempPath)

            # Validate the download isn't empty or corrupted (basic size check)
            $tempSize = (Get-Item $tempPath).Length
            if ($tempSize -lt 10000) {
                Remove-Item $tempPath -Force -ErrorAction SilentlyContinue
                throw "Downloaded file is too small ($tempSize bytes) — likely corrupted."
            }

            # Replace the installed script with the clean download
            Move-Item -Path $tempPath -Destination $script:InstallPath -Force
        ...
        # Silently skip if no internet — don't bother the user
    }
}
